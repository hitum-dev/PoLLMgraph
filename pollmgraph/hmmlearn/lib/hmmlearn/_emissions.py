...
class BaseGaussianHMM(_AbstractHMM):
    ...
    def _compute_log_likelihood(self, X):
        if np.isnan(self.means_).any():
            self.means_[np.isnan(self.means_)] = 0
        if np.isinf(self.means_).any():
            self.means_[np.isinf(self.means_)] = 0
        if np.isnan(self._covars_).any():
            self._covars_[np.isnan(self._covars_)] = 0
        if np.isinf(self._covars_).any():
            self._covars_[np.isinf(self._covars_)] = 0
