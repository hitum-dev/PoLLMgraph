...
class _AbstractHMM(BaseEstimator):
    ...
    def fit(self, X, lengths=None):
        ...
        previous_transmat_ = None
        for iter in tqdm(range(self.n_iter), desc="HMM training..."):
            previous_transmat_ = self.transmat_.copy()
            ...
            if (self.transmat_.sum(axis=1) == 0).any():
                self.transmat_ = previous_transmat_
                _log.warning("Reverting to previous transition matrix because no "
                     "transition from a state was ever observed.")
                break
        return self
    ...
    def _accumulate_sufficient_statistics_log(
        ...
        """
        stats['nobs'] += 1
        if 's' in self.params and len(posteriors) > 0:
            stats['start'] += posteriors[0]
        if 't' in self.params:
            n_samples, n_components = lattice.shape
            # when the sample is of length 1, it contains no transitions
